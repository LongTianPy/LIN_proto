...
def NewCount(filepath):
    ...
    os.system(cmd1)
    NewFiles = [f for f in listdir(filepath) if isfile(join(filepath, f)) and f.endswith('.fasta')]
    NewFiles.sort()
    OrigFiles = [f for f in listdir('/home/vinatzerlab/Data/kPALevaluation/Psy/') if isfile(join('/home/vinatzerlab/Data/kPALevaluation/Psy/',f)) and f.endswith('fasta')] # File path here should be the workspace where all the original genomes are in.
    OrigFiles.sort()
