...
def build_db():
	...
	c = conn.cursor()
	c.execute('CREATE DATABASE IF NOT EXISTS LINdb_test_Psy')
	c.execute('USE LINdb_test_Psy')
	...
	c.execute('CREATE TABLE User (User_ID int NOT NULL AUTO_INCREMENT,'
        ...
        'Email text NOT NULL,'
		'PRIMARY KEY (User_ID))')
    c.execute('CREATE TABLE AttributeValue (AttributeValue_ID int NOT NULL AUTO_INCREMENT,'
              'Genome_ID int NOT NULL,'
              'Interest_ID int NOT NULL,'
