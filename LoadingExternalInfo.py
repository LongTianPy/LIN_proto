...
def LoadInfo(file, db_cursor,genome_name, interest_id, user=1, private=True):
    ...
    for i in range(len(info_to_be_loaded)):
        query = "INSERT INTO AttributeValue (Attribute_ID, Genome_ID, Interest_ID, AttributeValue, User_ID, Private) values ({0}, {1}, {2}, '{3}', {4}, {5})".format(Attribute_IDs[i],Genome_ID, interest_id, info_to_be_loaded[i], user, private)
